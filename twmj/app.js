...
RocknCoder.Display = function() {
	var $displayControl,
		...
		setOperator = function(newOperator) {
				...
				calculate();
			//sweetAlert({
//  title:Math.floor(1 + $displayControl.value/5) + '隻' + $displayControl.value % 5,
//  showCancelButton: true,
//  confirmButtonText: '拉一半',
////  cancelButtonText: '取消',
//  imageUrl:"images/twmj1.png",
//  preConfirm: function() {
...
//}).then(function(isConfirm) {
 // if (isConfirm) {
  //  sweetAlert({
//title:'Test',
//(Math.ceil($displayControl.value/2 + 2.5) - Math.ceil($displayControl.value/2 + 2.5)% 5)/5 + '隻' + Math.ceil($displayControl.value/2 + 2.5)% 5,
//imageUrl:"images/twmj1.png"
//});
 // }
//});
	sweetAlert({title:Math.floor(1 + $displayControl.value/5) + '隻' + $displayControl.value % 5 + '\n拉多：' + (Math.ceil($displayControl.value/2 + 2.5) - Math.ceil($displayControl.value/2 + 2.5)% 5)/5 + '隻' + Math.ceil($displayControl.value/2 + 2.5)% 5 + '\n拉少：' + (Math.floor($displayControl.value/2 + 2.5) - Math.floor($displayControl.value/2 + 2.5)% 5)/5 + '隻' + Math.floor($displayControl.value/2 + 2.5)% 5, imageUrl:'images/twmj1.png', background:'#F8E0F1'});
				clearDisplay();
				return;
			}
